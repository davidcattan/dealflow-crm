...
export async function parseDealWorkbook(buffer: Buffer): Promise<ParsedDeal[]> {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    const dateRaw = cellToString(get(idx.date)).trim()
    const { iso: createdAt, year } = parseSlashDate(dateRaw)
    ...
    const activityScore =
      Number.isFinite(activityNum) && activityNum >= 1 && activityNum <= 10
    ...
    deals.push({
      ...
      activityScore,
      notes: cellToString(get(idx.notes)).trim() || null,
      createdAt,
      updates,
    })
  })
